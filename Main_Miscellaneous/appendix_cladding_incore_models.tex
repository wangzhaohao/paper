...
\begin{subequations}\label{eq:appendix_cladding_density_fits}
	\begin{align}
		\rho_{\mathrm{D9}}(T_C)
		&=7.98-4.3\times10^{-4}T_C,
		&&300<T_C<800\ ^\circ\mathrm C
		\label{eq:appendix_d9_density_fit}\\
		\rho_{\mathrm{HT9}}(T_C)
		&=7.778-3.07\times10^{-4}T_C,
		&&0\leq T_C\leq800\ ^\circ\mathrm C
		\label{eq:appendix_ht9_density_fit}
	\end{align}
\end{subequations}
...
\begin{subequations}\label{eq:appendix_ss316_legacy_elasticity}
	\begin{align}
		E_{316}^{\mathrm{legacy}}
		&=2.15946\times10^{11}-7.07727\times10^7T
		\label{eq:appendix_ss316_legacy_youngs_modulus}\\
		\nu_{316}^{\mathrm{legacy}}&=0.31
		\label{eq:appendix_ss316_legacy_poisson_ratio}
	\end{align}
\end{subequations}
...
\begin{subequations}\label{eq:appendix_ss316_swelling_parameters}
	\begin{align}
		\beta&=\frac{T-773}{100}
		\label{eq:appendix_ss316_swelling_beta}\\
		R(T)&=\exp\!\left(
		...
		\label{eq:appendix_ss316_swelling_R}\\
		\alpha&=0.75
		\label{eq:appendix_ss316_swelling_alpha}\\
		\tau(T)&=
		\begin{cases}
			6.58-0.566\beta,&T<848\ \mathrm{K}\\
			4.3105+2.46\beta,&T\geq848\ \mathrm{K}
		\end{cases}
		\label{eq:appendix_ss316_swelling_tau}
	\end{align}
\end{subequations}
采用增量更新时，空洞项的变化率为
\begin{equation}
...
D9的肿胀率、孕育期及固态密度修正为
\begin{subequations}\label{eq:appendix_d9_swelling_parameters}
	\begin{align}
		R_{\mathrm{D9}}(T)
		&=2.76\exp[-1.4\times10^{-4}(T-773.15)^2]
		\label{eq:appendix_d9_swelling_R}\\
		\alpha_{\mathrm{D9}}&=0.75,
		\qquad
		\tau_{\mathrm{D9}}=11.9
		\label{eq:appendix_d9_swelling_alpha_tau}\\
		D_{\mathrm{D9}}(T,\Phi)
		&=0.01(1-\exp[-30\Phi])
		(-1.7\times10^{-4}T+0.241)
		\label{eq:appendix_d9_solid_state_density_change}
	\end{align}
\end{subequations}
相应的增量形式为
\begin{equation}
...
HT9的模型参数为
\begin{subequations}\label{eq:appendix_ht9_swelling_parameters}
	\begin{align}
		R_{\mathrm{HT9}}(T)
		&=0.085\exp[-1.0\times10^{-4}(T-673)^2]
		\label{eq:appendix_ht9_swelling_R}\\
		\alpha_{\mathrm{HT9}}&=0.75,
		\qquad
		\tau_{\mathrm{HT9}}=14.2
		\label{eq:appendix_ht9_swelling_alpha_tau}\\
		D_{\mathrm{HT9}}(\Phi)
		&=0.0015[1-\exp(-0.1\Phi)]
		\label{eq:appendix_ht9_solid_state_density_change}
	\end{align}
\end{subequations}
其增量形式为
\begin{equation}
...
相应的特征应力和剪切模量为
\begin{subequations}\label{eq:appendix_cladding_characteristic_stress}
	\begin{align}
		\sigma^*_{\mathrm{D9}}
		&=450-240\tanh\!\left(\frac{T_C-760}{300}\right),
		&
		G_{\mathrm{D9}}&=78600-38.41T_C
		\label{eq:appendix_d9_characteristic_stress}\\
		\sigma^*_{\mathrm{HT9}}
		&=430-190\tanh\!\left(\frac{T_C-640}{225}\right),
		&
		G_{\mathrm{HT9}}&=89640-53.78T_C
		\label{eq:appendix_ht9_characteristic_stress}
	\end{align}
\end{subequations}
式中，应力和模量单位均为MPa。
...
\begin{subequations}\label{eq:appendix_d9_ht9_ultimate_strength}
	\begin{align}
		\sigma_{u,\mathrm{D9}}
		&=\sigma_{y,\mathrm{D9}}
		\left[
		...
		+1.055
		\right]
		\label{eq:appendix_d9_ultimate_strength}\\
		\sigma_{u,\mathrm{HT9}}
		&=\sigma_{y,\mathrm{HT9}}
		\left[
		1.1-0.1\tanh\!\left(\frac{\Delta T-200}{200}\right)
		\right]
		\label{eq:appendix_ht9_ultimate_strength}
	\end{align}
\end{subequations}
...
\begin{subequations}\label{eq:appendix_ss316_chaboche_isotropic_hardening}
	\begin{align}
		\dot r&=b(Q-r)\dot p
		\label{eq:appendix_ss316_chaboche_rate}\\
		r(p)&=Q[1-\exp(-bp)]
		\label{eq:appendix_ss316_chaboche_integrated}\\
		\sigma_y(p)&=\sigma_y^0+r(p)
		\label{eq:appendix_ss316_chaboche_yield}
	\end{align}
\end{subequations}
...
\begin{subequations}\label{eq:appendix_d9_ss316_973k_creep}
	\begin{align}
		\dot\varepsilon_{\mathrm{D9}}
		&=6\times10^{-30}\sigma^{9.2}
		\label{eq:appendix_d9_973k_creep}\\
		\dot\varepsilon_{316}
		&=6\times10^{-28}\sigma^9
		\label{eq:appendix_ss316_973k_creep}
	\end{align}
\end{subequations}
...
\end{equation}
各分量为
\begin{subequations}\label{eq:appendix_ht9_thermal_creep_components}
	\begin{align}
		\bar\varepsilon_{\mathrm{TP}}
		={}&\left[
		...
		\right]
		[1-e^{-C_4t}]
		\label{eq:appendix_ht9_primary_creep}\\
		\bar\varepsilon_{\mathrm{TS}}
		={}&\left[
		C_5e^{-Q_4/(RT)}\bar\sigma^2
		+C_6e^{-Q_5/(RT)}\bar\sigma^5
		\right]t
		\label{eq:appendix_ht9_secondary_creep}\\
		\bar\varepsilon_{\mathrm{TT}}
		={}&C_7e^{-Q_6/(RT)}\bar\sigma^{10}t^4
		\label{eq:appendix_ht9_tertiary_creep}
	\end{align}
\end{subequations}
...
\end{align}
其中
\begin{subequations}\label{eq:appendix_d9_transient_auxiliary_functions}
	\begin{align}
		\mathrm{TRAMP}
		&=-0.28+1.18\tanh[-0.5\ln(\dot T)-1]
		\label{eq:appendix_d9_tramp}\\
		\sigma^*
		={}&775-387.5
		...
		\tanh\!\left(\frac{\Phi}{2.5}\right)
		\label{eq:appendix_d9_transient_characteristic_stress}
	\end{align}
\end{subequations}
...
\end{equation}
高、低应力区的参数分别为
\begin{subequations}\label{eq:appendix_ht9_mfh_rupture_parameters}
	\begin{align}
		(A,B,C)_{\mathrm H}
		&=(-32.490,\ 57781,\ -11800)
		\label{eq:appendix_ht9_mfh_high_stress_parameters}\\
		(A,B,C)_{\mathrm L}
		&=(-35.173,\ 45858,\ -5563.1)
		\label{eq:appendix_ht9_mfh_low_stress_parameters}
	\end{align}
\end{subequations}
...
\begin{subequations}\label{eq:appendix_ht9_whc_failure_model}
	\begin{align}
		t_r&=\theta\exp\!\left(\frac{Q}{RT}\right)
		\label{eq:appendix_ht9_whc_rupture_time}\\
		\ln\theta
		&=A_T+12.47\ln\!\left[
		\ln\!\left(\frac{730}{\sigma_\theta}\right)
		\right]
		\label{eq:appendix_ht9_whc_dorn_parameter}\\
		A_T&=
		\begin{cases}
			24.942-0.153T_C
		...
		\end{cases}
		\label{eq:appendix_ht9_whc_temperature_factor}
	\end{align}
\end{subequations}
