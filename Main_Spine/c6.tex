...
本章采用分阶段研究策略。首先，在固定功率历史形状下改变整体功率倍率，构建POD系数插值和POD--GPR模型，以验证网格统一、POD降阶、系数预测和二维场恢复流程，并形成规则一维参数空间下的基准结果。在此基础上，进一步生成具有不同整体幅值、局部幅值、瞬态时刻和持续时间的功率历史，构建POD--LSTM时序代理模型。该模型以时间、瞬时功率、功率变化率和累积功率积分特征为序列输入，预测温度、径向位移和轴向位移的POD系数增量，并采用可训练的循环初始状态减小固定初始条件下的序列冷启动误差。
...
式中：$\alpha_g$为全寿期整体功率倍率，$\alpha_e$、$\alpha_m$和$\alpha_l$分别控制前期、中期和后期的局部功率水平，$\Delta t_{\mathrm{tr}}$表示关键瞬态发生时刻的平移量，$s_{\mathrm{tr}}$表示瞬态持续时间倍率，$\mathcal W$为分段幅值变换和时间变换算子。变功率历史生成程序采用的参数范围见表~\ref{tab:formal_power_parameter_ranges}。
...
数据集设计为40条训练历史、10条验证历史和10条独立盲测历史。所有样本均按照完整功率历史划分，而不是将同一工况的不同时间点随机拆分。训练历史用于构建POD基、计算标准化参数和更新网络权重；验证历史用于POD阶数选择、早停和有限超参数选择；盲测历史在POD阶数、网络结构和训练策略冻结后仅评价一次。
...
变功率历史数据集包含40条通过完整性检查的训练历史，因此单场训练快照数为$40\times109=4360$，对应快照矩阵尺寸为
...
\xsubsection{变功率数据集下的POD重新选阶}
{POD Rank Reselection for the Variable-Power Dataset}

前述$r_T=10$、$r_{u_r}=10$和$r_{u_z}=8$是固定功率倍率基准数据集的阶数，
用于插值和GPR模型。对于变功率历史数据集，本文仅使用40条训练历史重新
构造三个场的快照矩阵和POD基，并在10条验证历史上重新扫描阶数。POD--LSTM模型的总输出维数定义为
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}
		{>{\centering\arraybackslash}p{2.0cm}
			...
			X}
		\toprule
		场变量 & POD阶数 & 累计能量/\% & 验证平均相对$L_2$\% & 盲测平均相对$L_2$\% & 盲测最大RMSE \\
		\midrule
...
由表~\ref{tab:formal_pod_results}可见，温度、径向位移和轴向位移分别保留10、10和8阶模态后，验证集与盲测集的平均POD直接投影相对$L_2$误差均低于0.04\%。相应盲测最大瞬时RMSE分别为1.360~K、$0.060~\mu$m和$0.646~\mu$m，显著低于后续POD--LSTM端到端误差。因此，所构建的POD空间能够充分表示未见变功率工况，端到端模型误差主要来自POD系数的时序预测，而不是POD截断。

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.80\textwidth]
	{pod_lstm/formal_pod_rank_scan.png}
	\caption{验证集上的POD阶数扫描结果}
...
上述固定功率倍率结果构成无记忆代理模型的对照基线，其主要作用是验证POD系数预测与完整场恢复流程，并揭示仅依赖当前时刻和整体功率倍率的模型在快速功率变化及热--力状态转折附近仍会出现局部误差。由于POD--LSTM模型采用不同形状的变功率历史数据集，二者不直接比较绝对误差大小，而是分别用于评价规则单参数空间和历史依赖时序问题下的预测能力。
...
经验证集上的有限超参数选择后，POD--LSTM模型冻结为单层LSTM、32个隐藏单元、
...
\xsection{POD--LSTM模型预测结果与讨论}
{Prediction Results and Discussion of the POD--LSTM Model}
...
\xsubsection{数据集覆盖范围与POD投影精度}
{Dataset Coverage and POD Projection Accuracy}

变功率历史数据集由40条训练历史、10条验证历史和10条独立盲测历史组成。所有60条历史均已成功生成109步序列文件。由case manifest统计可知，样本的累积线功率积分相对基准值覆盖0.858--1.146，最大功率相对基准峰值覆盖0.892--1.140。训练集的最大功率变化率比值为0.828--1.173；盲测工况\texttt{case\_lstm\_bl\_0002}的该比值达到1.863，因此独立盲测集不仅包含训练参数组合的重新组合，还包含有限程度的功率斜率外推。

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{pod_lstm/power_history_overview.png}
	\caption{POD--LSTM数据集的变功率历史}
	\label{fig:power_history_overview}
\end{figure}

由表~\ref{tab:formal_pod_results}可知，独立盲测集中温度、径向位移和轴向位移
的POD直接投影平均相对$L_2$误差分别为0.0192\%、0.0380\%和0.0200\%，
显著低于后续端到端POD--LSTM盲测误差。由此可确认，所构建的POD空间对未见
...
从10条盲测历史的平均结果看，温度、径向位移和轴向位移的平均相对$L_2$误差分别为1.256\%、1.528\%和2.549\%，表明冻结后的POD--LSTM能够在未见功率历史上保持较好的完整场预测精度。与此同时，三个场的最大相对$L_2$误差分别达到15.144\%、10.987\%和17.913\%，说明总体误差并非在全寿期内均匀分布，而是存在少量局部高误差时刻。三个场的最大节点绝对误差分别为193.43~K、$24.32~\mu$m和$462.40~\mu$m。结合前述POD直接投影结果可知，POD截断误差显著低于上述端到端预测误差，因此当前误差主要来源于POD系数的时序预测，而不是低维空间缺少必要的空间模态。
...
0.991、0.940和0.926，均处于训练数据的覆盖范围内。因此，本算例用于
...
POD--LSTM模型可处理训练数据包络内的不同功率史形状，但并不意味着能够对
任意幅值、变化率或持续时间的未见历史进行无条件外推。新功率历史的峰值、
因此，本文给出的有效适用范围限定为新功率历史的峰值、变化率、关键瞬态时刻、平台持续时间和累积功率积分特征处于训练数据覆盖范围内或其邻近区域。

当前POD基建立在固定5717节点网格和固定燃料棒几何上。改变燃料半径、包壳
厚度、初始间隙或燃料棒长度会改变节点坐标和物理对应关系，需要首先将不同
几何上的场映射至统一参考域，不能直接混合现有快照矩阵。

冷却剂温度和压力在当前POD--LSTM模型中保持不变。将冷却剂温度历史扩展为新的
...
变功率历史数据集包含40条训练历史、
10条验证历史和10条
独立盲测历史。基于该数据集重新选取的温度、径向位移和轴向位移POD阶数分别为
...
独立盲测中的最大误差集中在\texttt{case\_lstm\_bl\_0006}的寿期后段，而其余
大部分寿期内的温度和位移空间分布
