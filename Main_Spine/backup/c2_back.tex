...
\xsubsection{裂变气体程序}{Sciantix Fission Gas Released Program}
...
\begin{align*}
	\Omega         & : 9                                          \\
	a_1            & = 134\exp\left(-\frac{18800}{RT}\right)     \\
...
积分形式：
\begin{multline}
	\bar{\varepsilon} = A_1 \exp\left(-\frac{A_2}{T}\right) \left[1 - \exp\left(-3\Phi\right)\right] \bar{\sigma} + A_3 \Phi\bar{\sigma} \\
	+ A_6 t \left(\frac{\bar{\sigma}}{Y}\right)^4 \exp\left(-\frac{A_7}{T}\right) + 2.2 R(T) \left(\Omega \right)^2 \bar{\sigma} \log_e\left[\cosh\left(\frac{\Phi}{\Omega}\right)\right] \\
	+ A_4 t \bar{\sigma} \exp\left(-\frac{A_5}{T}\right)
\end{multline}

需要注意，$A_6$和$A_4$开头的代表热蠕变。\\
蠕变率形式：
\begin{multline}
	\dot{\bar{\varepsilon}} = \left[3A_1 \exp\left(-\frac{A_2}{T}\right) \exp\left(-3\Phi\right) \bar{\sigma} + A_3 \bar{\sigma} + 2.2 R(T)\Omega \bar{\sigma} \tanh\left(\frac{\Phi}{\Omega}\right)\right] \phi \times 10^{-7} \\
	+ A_6 \left(\frac{\bar{\sigma}}{Y}\right)^4 \exp\left(-\frac{A_7}{T}\right) + A_4 \bar{\sigma} \exp\left(-\frac{A_5}{T}\right)
\end{multline}
